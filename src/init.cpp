...
#include "zambk/accumulatorcheckpoints.h"
...
void PrepareShutdown()
{
    ...
    RenameThread("ambankcoin-shutoff");
...
void Shutdown()
{
    ...
    if (nLocalServices == NODE_BLOOM_LIGHT_ZC) {
        lightWorker.StopLightZambkThread();
    }
#ifdef ENABLE_WALLET
    delete pwalletMain;
    pwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "ambankcoin.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "ambankcoind.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the AMBK and zAMBK money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 34001, 34003));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in AMBK/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in AMBK/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    std::string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, ambankcoin, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero, precompute, staking)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in AMBK/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all AMBANKCOIN specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-ambkstake=<n>", strprintf(_("Enable or disable staking functionality for AMBK inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zambkstake=<n>", strprintf(_("Enable or disable staking functionality for zAMBK inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzambk=<n>", strprintf(_("Enable automatic wallet backups triggered after each zAMBK minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-precompute=<n>", strprintf(_("Enable precomputation of zAMBK spends and stakes (0-1, default %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-zambkbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zAMBK backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 34002, 34004));
...
{
    RenameThread("ambankcoin-loadblk");
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. AMBANKCOIN Core is shutting down."));
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. AMBANKCOIN Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //AMBANKCOIN: Load Accumulator Checkpoints according to network (main/test/regtest)
        ...
        do {
            ...
            try {
                ...
                if(Params().NetworkID() == CBaseChainParams::MAIN && chainHeight > Params().Zerocoin_Block_EndFakeSerial()) {
                    ...
                    // CAmount zambkSupplyCheckpoint = Params().GetSupplyBeforeFakeSerial() + GetWrapppedSerialInflationAmount();

                    // if (pblockindex->GetZerocoinSupply() < zambkSupplyCheckpoint) {
                    //     // Trigger reindex due wrapping serials
                    //     LogPrintf("Current GetZerocoinSupply: %d vs %d\n", pblockindex->GetZerocoinSupply()/COIN , zambkSupplyCheckpoint/COIN);
                    //     reindexDueWrappedSerials = true;
                    // } else if (pblockindex->GetZerocoinSupply() > zambkSupplyCheckpoint) {
                    //     // Trigger global zAMBK reindex
                ...
                if (GetBoolArg("-reindexmoneysupply", false) || reindexZerocoin) {
                    if (chainHeight > Params().Zerocoin_StartHeight()) {
                        RecalculateZAMBKMinted();
                        RecalculateZAMBKSpent();
                    }
                    // Recalculate from the zerocoin activation or from scratch.
                    RecalculateAMBKSupply(reindexZerocoin ? Params().Zerocoin_StartHeight() : 1);
                }
                ...
                if(Params().NetworkID() == CBaseChainParams::MAIN && chainHeight > Params().Zerocoin_Block_EndFakeSerial()) {
                    ...
                    // CAmount zambkSupplyCheckpoint = Params().GetSupplyBeforeFakeSerial() + GetWrapppedSerialInflationAmount();
                    // if (pblockindex->GetZerocoinSupply() != zambkSupplyCheckpoint)
                    //     return InitError(strprintf("ZerocoinSupply Recalculation failed: %d vs %d", pblockindex->GetZerocoinSupply()/COIN , zambkSupplyCheckpoint/COIN));
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainHeight > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // AMBANKCOIN: recalculate Accumulator Checkpoints that failed to database properly
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of AMBANKCOIN Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart AMBANKCOIN Core to complete") << "\n";
        ...
        zwalletMain = new CzAMBKWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zAMBKWallet
        uiInterface.InitMessage(_("Syncing zAMBK wallet..."));

        pwalletMain->InitAutoConvertAddresses();

        bool fEnableZAmbkBackups = GetBoolArg("-backupzambk", true);
        pwalletMain->setZAmbkAutoBackups(fEnableZAmbkBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zambkTracker->Init();
    ...
    /* Denominations
       ...
       For example:
       1AMBK+1000 == (.1AMBK+100)*10
       10AMBK+10000 == (1AMBK+1000)*10
    */
    ...
    if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
        ...
        lightWorker.StartLightZambkThread(threadGroup);
    }
    ...
    if (pwalletMain) {
        ...
        if (GetBoolArg("-precompute", false)) {
            // Run a thread to precompute any zAMBK spends
