...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    CAmount nAmbk = wtx.GetAmbk(ISMINE_ALL);
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        libzerocoin::CoinSpend zcspend = wtx.HasZerocoinPublicSpendInputs() ? ZAMBKModule::parseCoinSpend(wtx.vin[0]) : TxInToZerocoinSpend(wtx.vin[0]);
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zambkTracker->HasMintTx(hash))) {
            //zAMBK stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZAMBK;
            ...
            if (ExtractDestination(wtx.vout[nIndexMN].scriptPubKey, destMN) && IsMine(*wallet, destMN)) {
                ...
                sub.ambk = wtx.vout[nIndexMN].nValue;
            }
        }
        ...
        for (const CTxOut& txout : wtx.vout) {
            ...
            if (txout.IsZerocoinMint()) {
                ...
                TransactionRecord sub(hash, nTime, wtx.GetTotalSize());
                sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
                sub.type = TransactionRecord::ZerocoinSpend_Change_zAmbk;
                sub.address = mapValue["zerocoinmint"];
                if (!fFeeAssigned) {
        ...
        if (fAllFromMeDenom && fAllToMeDenom && nFromMe * nToMe) {
            parts.append(TransactionRecord(hash, nTime, wtx.GetTotalSize(), TransactionRecord::ObfuscationDenominate, "", -nDebit, nAmbk));
...
bool IsZAMBKType(TransactionRecord::Type type)
{
    switch (type) {
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZAMBK || type == TransactionRecord::MNReward) {
