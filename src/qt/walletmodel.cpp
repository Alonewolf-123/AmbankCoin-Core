...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 AMBK.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
bool WalletModel::createZambkSpend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackZambk(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        bool fMintChange,
        bool fMinimizeChange,
...
static void NotifyzAMBKReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzAMBKReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzAMBKReset.connect(boost::bind(NotifyzAMBKReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzAMBKReset.disconnect(boost::bind(NotifyzAMBKReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zambkTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
