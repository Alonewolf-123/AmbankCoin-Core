...
QString TransactionDesc::toHTML(CWallet* wallet, CWalletTx& wtx, TransactionRecord* rec, int unit)
{
    ...
    CAmount nNet = rec->ambk + rec->debit;
    ...
    // Amount
    //
    if (wtx.IsCoinBase() && rec->ambk == 0) {
        //
        // Coinbase
        //
        CAmount nUnmatured = 0;
        for (const CTxOut& txout : wtx.vout)
            nUnmatured += wallet->GetAmbk(txout, ISMINE_ALL);
        strHTML += "<b>" + tr("Ambk") + ":</b> ";
        ...
        // Ambk
        //
        strHTML += "<b>" + tr("Ambk") + ":</b> " + BitcoinUnits::formatHtmlWithUnit(unit, nNet) + "<br>";
    } else {
        ...
        if (fAllFromMe) {
            ...
            for (const CTxOut& txout : wtx.vout) {
                ...
                if (toSelf)
                    strHTML += "<b>" + tr("Ambk") + ":</b> " + BitcoinUnits::formatHtmlWithUnit(unit, txout.nValue) + "<br>";
            }

            if (fAllToMe) {
                // Payment to self
                CAmount nChange = wtx.GetChange();
                CAmount nValue = rec->ambk - nChange;
                strHTML += "<b>" + tr("Total debit") + ":</b> " + BitcoinUnits::formatHtmlWithUnit(unit, -nValue) + "<br>";
                strHTML += "<b>" + tr("Total ambk") + ":</b> " + BitcoinUnits::formatHtmlWithUnit(unit, nValue) + "<br>";
            }
            ...
            for (const CTxOut& txout : wtx.vout)
                if (wallet->IsMine(txout))
                    strHTML += "<b>" + tr("Ambk") + ":</b> " + BitcoinUnits::formatHtmlWithUnit(unit, wallet->GetAmbk(txout, ISMINE_ALL)) + "<br>";
        }
    }
    ...
    // Message from normal ambankcoin:URI (ambankcoin:XyZ...?message=example)
