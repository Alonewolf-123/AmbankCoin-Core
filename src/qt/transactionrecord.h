...
public:
    ...
    TransactionRecord(unsigned int size) : hash(), time(0), type(Other), address(""), debit(0), ambk(0), size(size), idx(0)
    {
    }

    TransactionRecord(uint256 hash, qint64 time, unsigned int size) : hash(hash), time(time), type(Other), address(""), debit(0),
                                                   ambk(0), size(size), idx(0)
    {
    }

    TransactionRecord(uint256 hash, qint64 time, unsigned int size, Type type, const std::string& address, const CAmount& debit, const CAmount& ambk) : hash(hash), time(time), type(type), address(address), debit(debit), ambk(ambk),
                                                                                                                                       size(size), idx(0)
    {
    }
