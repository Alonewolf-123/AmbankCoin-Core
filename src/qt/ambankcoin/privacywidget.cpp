...
#include "qt/ambankcoin/privacywidget.h"
#include "qt/ambankcoin/forms/ui_privacywidget.h"
#include "qt/ambankcoin/qtutils.h"
#include "guiutil.h"
#include "qt/ambankcoin/denomgenerationdialog.h"
...
PrivacyWidget::PrivacyWidget(AMBANKCOINGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zAMBK anonymizes your AMBK by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zAMBK or convert back to AMBK"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 AMBK ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zAMBK");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zAMBK");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zAMBK");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zAMBK");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zAMBK");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zAMBK");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zAMBK");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zAMBK");
    ...
    // Only Convert to AMBK enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzAMBK->setTitleClassAndText("btn-title-grey", "Total 0 zAMBK");
    ui->btnTotalzAMBK->setSubTitleClassAndText("text-subtitle", "Show denominations of zAMBK owned.");
    ui->btnTotalzAMBK->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select AMBK outputs to mint into zAMBK.");
    ...
    connect(ui->btnTotalzAMBK, SIGNAL(clicked()), this, SLOT(onTotalZambkClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zAMBK");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of AMBK to mint into zAMBK"));
    }else{
        btnText = tr("Convert back to AMBK");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zAMBK to convert back into AMBK"));
    }
...
void PrivacyWidget::onTotalZambkClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzAMBK->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzAMBK->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zAMBK is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zAMBK, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zAMBK").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zAMBK minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZambk(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zAMBK converted back to AMBK"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zAMBK </b>";
    ...
    ui->btnTotalzAMBK->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
