...
TopBar::TopBar(AMBANKCOINGUI* _mainWindow, QWidget* parent) : PWidget(_mainWindow, parent),
                                                             ui(new Ui::TopBar)
{
    ui->setupUi(this);
    ...
    setCssProperty({ui->labelAmountTopAmbk, ui->labelAmountTopzAmbk}, "amount-small-topbar");
    setCssProperty({ui->labelAmountAmbk, ui->labelAmountzAmbk}, "amount-topbar");
    setCssProperty({ui->labelPendingAmbk, ui->labelPendingzAmbk, ui->labelImmatureAmbk, ui->labelImmaturezAmbk}, "amount-small-topbar");

    ui->labelAmountTopzAmbk->setVisible(false);
    ui->label_16->setVisible(false);
    ui->labelTitle2->setVisible(false);
    ui->labelAmountzAmbk->setVisible(false);
    ui->labelTitle5->setVisible(false);
    ui->labelPendingzAmbk->setVisible(false);
    ui->labelTitle6->setVisible(false);
    ui->labelImmaturezAmbk->setVisible(false);
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& zerocoinBalance, const CAmount& unconfirmedZerocoinBalance, const CAmount& immatureZerocoinBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance, const CAmount& delegatedBalance, const CAmount& coldStakedBalance)
{
    ...
    CAmount ambkAvailableBalance = balance + delegatedBalance - nLockedBalance;

    // zAMBK Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalAmbk = GUIUtil::formatBalance(ambkAvailableBalance, nDisplayUnit);
    QString totalzAmbk = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    // Top
    ui->labelAmountTopAmbk->setText(totalAmbk);
    ui->labelAmountTopzAmbk->setText(totalzAmbk);

    // Expanded
    ui->labelAmountAmbk->setText(totalAmbk);
    ui->labelAmountzAmbk->setText(totalzAmbk);

    ui->labelPendingAmbk->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelPendingzAmbk->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmatureAmbk->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    ui->labelImmaturezAmbk->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
