    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("AMBK");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zAMBK");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (AMBK) or private coins (zAMBK)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Enter a AMBANKCOIN address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 AMBK");
...
void SendWidget::refreshView(){
    ...
    if(ui->pushLeft->isChecked()){
        btnText = tr("Send AMBK");
        ui->pushButtonAddRecipient->setVisible(true);
    }else{
        btnText = tr("Send zAMBK");
...
void SendWidget::refreshAmounts() {
    ...
    bool isZambk = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZambk));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZambk ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendAmbk = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendAmbk ? AskPassphraseDialog::Context::Send_AMBK : AskPassphraseDialog::Context::Send_zAMBK, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendAmbk) ? send(recipients) : sendZambk(recipients)) {
...
bool SendWidget::sendZambk(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zAMBK is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zAMBK selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZAmbkControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZAmbkControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZambk(
        ...
        inform(tr("zAMBK transaction sent!"));
        ZAmbkControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZAMBK_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zAMBK require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zAMBK transaction
        ...
        emit message("zAMBK transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isAMBK){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any AMBK to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ZAmbkControlDialog *zAmbkControl = new ZAmbkControlDialog(this);
            zAmbkControl->setModel(walletModel);
            zAmbkControl->exec();
            ui->btnCoinControl->setActive(!ZAmbkControlDialog::setSelectedMints.empty());
            zAmbkControl->deleteLater();
        } else {
            inform(tr("You don't have any zAMBK in your balance to select."));
        }
    }
}
...
void SendWidget::onAMBKSelected(bool _isAMBK){
    isAMBK = _isAMBK;
    setCssProperty(coinIcon, _isAMBK ? "coin-icon-ambk" : "coin-icon-zambk");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress ambkAdd = CBitcoinAddress(address.toStdString());
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(ambkAdd.Get(), dialog->getLabel().toStdString(), "send")) {
