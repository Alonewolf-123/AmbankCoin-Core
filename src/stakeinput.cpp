...
CZAmbkStake::CZAmbkStake(const libzerocoin::CoinSpend& spend)
{
...
int CZAmbkStake::GetChecksumHeightFromMint()
{
...
int CZAmbkStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZAmbkStake::GetChecksum()
{
    return nChecksum;
}

// The zAMBK block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZAmbkStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZAmbkStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zAMBK staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CZAmbkStake::GetUniqueness()
{
    //The unique identifier for a zAMBK is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZAmbkStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZAmbkStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zAMBK that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZAMBKOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zAMBK output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zAMBK", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZAMBKOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zAMBK output", __func__);
...
bool CZAmbkStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZAmbkStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzAMBKTracker* zambkTracker = pwallet->zambkTracker.get();
    CMintMeta meta;
    if (!zambkTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zambkTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

//!AMBK Stake
bool CAmbkStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CAmbkStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}
...
CAmount CAmbkStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CAmbkStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CAmbkStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CAmbkStake::GetUniqueness()
{
...
CBlockIndex* CAmbkStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
