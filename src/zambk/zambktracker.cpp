...
CzAMBKTracker::CzAMBKTracker(std::string strWalletFile)
{
...
CzAMBKTracker::~CzAMBKTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzAMBKTracker::Init()
{
...
bool CzAMBKTracker::Archive(CMintMeta& meta)
{
...
bool CzAMBKTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzAMBKTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzAMBKTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzAMBKTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzAMBKTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzAMBKTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzAMBKTracker::GetSerialHashes()
{
...
CAmount CzAMBKTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzAMBKTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzAMBKTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzAMBKTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzAMBKTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzAMBKTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzAMBKTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzAMBKTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzAMBKTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzAMBKTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzAMBKTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzAMBKWallet* zAMBKWallet)
{
    bool iszAMBKWalletInitialized = (NULL != zAMBKWallet);
    CMintMeta meta;
    ...
    if (! iszAMBKWalletInitialized)
        zAMBKWallet = new CzAMBKWallet(strWalletFile);
    meta.isSeedCorrect = zAMBKWallet->CheckSeed(dMint);
    if (! iszAMBKWalletInitialized)
        delete zAMBKWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzAMBKTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzAMBKTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzAMBKTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzAMBKTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzAMBKTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzAMBKTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzAMBKWallet* zAMBKWallet = new CzAMBKWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zAMBKWallet);
        }
        delete zAMBKWallet;
        LogPrint("zero", "%s: added %d dzambk from DB\n", __func__, listDeterministicDB.size());
    }
