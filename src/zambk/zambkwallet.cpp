...
CzAMBKWallet::CzAMBKWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zambk seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZAMBKSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZAMBKSeed_deprecated()) {
                    LogPrintf("%s: Updated zAMBK seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zambk seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zambk wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzAMBKWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZAMBKCount(nCountLastUsed);
    else if (!walletdb.ReadZAMBKCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzAMBKWallet::Lock()
{
    seedMaster = 0;
}

void CzAMBKWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzAMBKWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZAMBK(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzAMBKWallet::LoadMintPoolFromDB()
{
...
void CzAMBKWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzAMBKWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzAMBKWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zambkTracker->HasPubcoinHash(pMint.first)) {
...
bool CzAMBKWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZAMBK(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zambkTracker which also adds to database
    pwalletMain->zambkTracker->Add(dMint, true);
...
void CzAMBKWallet::SeedToZAMBK(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzAMBKWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzAMBKWallet::GenerateDeterministicZAMBK(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzAMBKWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzAMBKWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzAMBKWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
