...
CAmount CWalletDB::GetAccountAmbkDebit(const std::string& strAccount)
{
    std::list<CAccountingEntry> entries;
    ListAccountAmbkDebit(strAccount, entries);

    CAmount nAmbkDebit = 0;
    for (const CAccountingEntry& entry : entries)
        nAmbkDebit += entry.nAmbkDebit;

    return nAmbkDebit;
}

void CWalletDB::ListAccountAmbkDebit(const std::string& strAccount, std::list<CAccountingEntry>& entries)
{
    bool fAllAccounts = (strAccount == "*");

    Dbc* pcursor = GetCursor();
    if (!pcursor)
        throw std::runtime_error("CWalletDB::ListAccountAmbkDebit() : cannot create DB cursor");
    unsigned int fFlags = DB_SET_RANGE;
    while (true) {
        // Read next record
        CDataStream ssKey(SER_DISK, CLIENT_VERSION);
        ...
        if (ret == DB_NOTFOUND)
            ...
            pcursor->close();
            throw std::runtime_error("CWalletDB::ListAccountAmbkDebit() : error scanning DB");
        }
...
DBErrors CWalletDB::ReorderTransactions(CWallet* pwallet)
{
    ...
    ListAccountAmbkDebit("", acentries);
...
DBErrors CWalletDB::LoadWallet(CWallet* pwallet)
{
    ...
    ListAccountAmbkDebit("*", pwallet->laccentries);
...
void ThreadFlushWalletDB(const std::string& strFile)
{
    ...
    RenameThread("ambankcoin-wallet");
...
bool CWalletDB::WriteDeterministicMint(const CDeterministicMint& dMint)
{
    ...
    return Write(std::make_pair(std::string("dzambk"), hash), dMint, true);
}

bool CWalletDB::ReadDeterministicMint(const uint256& hashPubcoin, CDeterministicMint& dMint)
{
    return Read(std::make_pair(std::string("dzambk"), hashPubcoin), dMint);
}

bool CWalletDB::EraseDeterministicMint(const uint256& hashPubcoin)
{
    return Erase(std::make_pair(std::string("dzambk"), hashPubcoin));
}
...
bool CWalletDB::ArchiveDeterministicOrphan(const CDeterministicMint& dMint)
{
    ...
    if (!Erase(std::make_pair(std::string("dzambk"), dMint.GetPubcoinHash())))
...
bool CWalletDB::WriteZAMBKSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed)
{
...
bool CWalletDB::EraseZAMBKSeed()
{
    uint256 hash;
    if(!ReadCurrentSeedHash(hash)){
        return error("Failed to read a current seed hash");
    }
    if(!WriteZAMBKSeed(hash, ToByteVector(base_uint<256>(0) << 256))) {
...
bool CWalletDB::EraseZAMBKSeed_deprecated()
{
    return Erase(std::string("dzs"));
}

bool CWalletDB::ReadZAMBKSeed(const uint256& hashSeed, std::vector<unsigned char>& seed)
{
    return Read(std::make_pair(std::string("dzs"), hashSeed), seed);
}

bool CWalletDB::ReadZAMBKSeed_deprecated(uint256& seed)
{
    return Read(std::string("dzs"), seed);
}

bool CWalletDB::WriteZAMBKCount(const uint32_t& nCount)
{
    return Write(std::string("dzc"), nCount);
}

bool CWalletDB::ReadZAMBKCount(uint32_t& nCount)
{
...
std::list<CDeterministicMint> CWalletDB::ListDeterministicMints()
{
    ...
    for (;;)
    {
        ...
        CDataStream ssKey(SER_DISK, CLIENT_VERSION);
        if (fFlags == DB_SET_RANGE)
            ssKey << make_pair(std::string("dzambk"), uint256(0));
        CDataStream ssValue(SER_DISK, CLIENT_VERSION);
        int ret = ReadAtCursor(pcursor, ssKey, ssValue, fFlags);
        fFlags = DB_NEXT;
        if (ret == DB_NOTFOUND)
