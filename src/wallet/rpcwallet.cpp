...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "so payments received with the address will be ambked to 'account'.\n"
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturns the current AMBANKCOIN address for receiving payments to this account.\n"

            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"

            "\nResult:\n"
            "\"ambankcoinaddress\"   (string) The account ambankcoin address\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            "setaccount \"ambankcoinaddress\" \"account\"\n"
            "\nSets the account associated with the given address.\n"

            "\nArguments:\n"
            "1. \"ambankcoinaddress\"  (string, required) The ambankcoin address to be associated with an account.\n"
...
UniValue getaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "getaccount \"ambankcoinaddress\"\n"
            "\nReturns the account associated with the given address.\n"

            "\nArguments:\n"
            "1. \"ambankcoinaddress\"  (string, required) The ambankcoin address for account lookup.\n"
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"ambankcoinaddress\"  (string) a ambankcoin address associated with the given account\n"
            "  ,...\n"
            "]\n"
...
UniValue sendtoaddressix(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw std::runtime_error(
            "sendtoaddressix \"ambankcoinaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassphrase() + "\n"
...
UniValue getreceivedbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in AMBK received for this account.\n"
...
CAmount GetAccountBalance(CWalletDB& walletdb, const std::string& strAccount, int nMinDepth, const isminefilter& filter)
{
    ...
    if (strAccount != "*")
        nBalance += walletdb.GetAccountAmbkDebit(strAccount);
...
UniValue movecmd(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw std::runtime_error(
            ...
            "3. amount            (numeric, required) Quantity of AMBK to move between accounts.\n"
            ...
            "\nExamples:\n"
            "\nMove 0.01 AMBK from the default account to the account named tabby\n" +
            HelpExampleCli("move", "\"\" \"tabby\" 0.01") +
            "\nMove 0.01 AMBK from timotei to akiko with a comment\n" +
    ...
    // Ambk
    CAccountingEntry ambk;
    ambk.nOrderPos = pwalletMain->IncOrderPosNext(&walletdb);
    ambk.strAccount = strTo;
    ambk.nAmbkDebit = nAmount;
    ambk.nTime = nNow;
    ambk.strOtherAccount = strFrom;
    ambk.strComment = strComment;
    pwalletMain->AddAccountingEntry(ambk, walletdb);
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw std::runtime_error(
            "sendfrom \"fromaccount\" \"toambankcoinaddress\" amount ( minconf \"comment\" \"comment-to\" includeDelegated)\n"
            "\nSent an amount from an account to a ambankcoin address.\n"
            "The amount is a real and is rounded to the nearest 0.00000001." +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"toambankcoinaddress\"  (string, required) The ambankcoin address to send funds to.\n"
            "3. amount                (numeric, required) The amount in AMBK. (transaction fee is added on top).\n"
            "4. minconf               (numeric, optional, default=1) Only use funds with at least this many confirmations.\n"
            ...
            "\nExamples:\n"
            "\nSend 0.01 AMBK from the default account to the address, must have at least 1 confirmation\n" +
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The ambankcoin address is the key, the numeric amount in AMBK is the value\n"
            "      ,...\n"
            "    }\n"
...
UniValue listcoldutxos(const UniValue& params, bool fHelp)
{
    ...
    for (std::map<uint256, CWalletTx>::const_iterator it =
        ...
        if(pcoin->GetColdStakingAmbk() == 0 && pcoin->GetStakeDelegationAmbk() == 0)
            continue;
...
void AcentryToJSON(const CAccountingEntry& acentry, const std::string& strAccount, UniValue& ret)
{
    ...
    if (fAllAccounts || acentry.strAccount == strAccount) {
        ...
        entry.push_back(Pair("amount", ValueFromAmount(acentry.nAmbkDebit)));
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 6)
        throw std::runtime_error(
            ...
            "    \"address\":\"ambankcoinaddress\",    (string) The ambankcoin address of the transaction. Not present for \n"
            "                                                move transactions (category = move).\n"
            "    \"category\":\"send|receive|move\", (string) The transaction category. 'move' is a local (off blockchain)\n"
            "                                                transaction between accounts, and not associated with an address,\n"
            "                                                transaction id or block. 'send' and 'receive' transactions are \n"
            "                                                associated with an address, transaction id and block details\n"
            "    \"amount\": x.xxx,          (numeric) The amount in AMBK. This is negative for the 'send' category, and for the\n"
...
UniValue listaccounts(const UniValue& params, bool fHelp)
{
    ...
    for (const CAccountingEntry& entry : acentries)
        mapAccountBalances[entry.strAccount] += entry.nAmbkDebit;
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    ...
    CAmount nAmbk = wtx.GetAmbk(filter);
    CAmount nDebit = wtx.GetDebit(filter);
    CAmount nNet = nAmbk - nDebit;
...
UniValue settxfee(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in AMBK/kB rounded to the nearest 0.00000001\n"
...
UniValue multisend(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw std::runtime_error(
            ...
            "multisend <AMBANKCOIN Address> <percent>\n"
...
//         throw std::runtime_error(
//             "getzerocoinbalance\n"
//             "\nReturn the wallet's total zAMBK balance.\n" +
//             HelpRequiringPassphrase() + "\n"

//             "\nResult:\n"
//             "amount         (numeric) Total zAMBK balance.\n"
...
//             "\nList all zAMBK mints in the wallet.\n" +
//             HelpRequiringPassphrase() + "\n"
...
//             "    \"zAMBK ID\": \"xxx\",   (string) Pubcoin in hex format.\n"
...
//     CWalletDB walletdb(pwalletMain->strWalletFile);
//     std::set<CMintMeta> setMints = pwalletMain->zambkTracker->ListMints(true, fMatureOnly, true);
...
//             objMint.push_back(Pair("zAMBK ID", m.hashPubcoin.GetHex()));     // PubCoin
...
//                     pwalletMain->zambkTracker->UpdateState(m);
//                 }
//             }
...
//     CWalletDB walletdb(pwalletMain->strWalletFile);
//     std::set<CMintMeta> setMints = pwalletMain->zambkTracker->ListMints(true, true, true);
...
//         throw std::runtime_error(
//             "listspentzerocoins\n"
//             "\nList all the spent zAMBK mints in the wallet.\n" +
//             HelpRequiringPassphrase() + "\n"
...
//             "\nMint the specified zAMBK amount\n" +
//             HelpRequiringPassphrase() + "\n"

//             "\nArguments:\n"
//             "1. amount      (numeric, required) Enter an amount of Ambk to convert to zAMBK\n"
...
//         throw JSONRPCError(RPC_WALLET_ERROR, "zAMBK minting is DISABLED");
...
//         throw JSONRPCError(RPC_WALLET_ERROR, "zAMBK is currently disabled due to maintenance.");
...
//             "    {\n"
//             "      \"value\": amount,         (numeric) Value in AMBK.\n"
//             "      \"address\": \"xxx\"         (string) AMBK address or \"zerocoinmint\" for reminted change.\n"
//             "    }\n"
//             "    ,...\n"
//             "  ]\n"
//             "}\n"
...
//     bool fMintChange = params[1].get_bool();        // Mint change to zAMBK
//     if (fMintChange && Params().NetworkID() != CBaseChainParams::REGTEST)
//         throw JSONRPCError(RPC_WALLET_ERROR, "zAMBK minting is DISABLED, cannot mint change");
...
//         throw JSONRPCError(RPC_WALLET_ERROR, "zAMBK old spend only available in regtest for tests purposes");
//     }

//     return DoZambkSpend(nAmount, fMintChange, fMinimizeChange, vMintsSelected, address_str, ispublicspend);
// }
...
//             "\nSpend zAMBK mints to a AMBK address.\n" +
//             HelpRequiringPassphrase() + "\n"
...
//     return DoZambkSpend(nAmount, false, true, vMintsSelected, address_str);
// }


// extern UniValue DoZambkSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool ispublicspend)
// {
...
//     CWalletDB walletdb(pwalletMain->strWalletFile);
//     CzAMBKTracker* zambkTracker = pwalletMain->zambkTracker.get();
...
//     CWalletDB walletdb(pwalletMain->strWalletFile);
//     CzAMBKTracker* zambkTracker = pwalletMain->zambkTracker.get();
//     std::set<CMintMeta> setMints = zambkTracker->ListMints(false, false, false);
...
//             "2. \"denomination\"         (integer, optional) Export a specific denomination of zAMBK\n"

//             "\nResult:\n"
//             "[                   (array of json object)\n"
//             "  {\n"
//             "    \"id\": \"serial hash\",  (string) the mint's zAMBK serial hash \n"
...
//             "    \"v\": version,   (numeric) The version of the zAMBK\n"
//             "    \"k\": \"privkey\"  (string) The zAMBK private key (V2+ zAMBK only)\n"
//             "  }\n"
//             "  ,...\n"
//             "]\n"
...
//     CzAMBKTracker* zambkTracker = pwalletMain->zambkTracker.get();
//     std::set<CMintMeta> setMints = zambkTracker->ListMints(!fIncludeSpent, false, false);
...
//             "  \"value\": amount    (numeric) The total zAMBK value of zerocoin mints that were added\n"
//             "}\n"
...
//         pwalletMain->zambkTracker->Add(mint, true);
//         count++;
...
//         throw std::runtime_error(
//             "reconsiderzerocoins\n"
//             "\nCheck archived zAMBK list to see if any mints were added to the blockchain.\n" +
//             HelpRequiringPassphrase() + "\n"
...
// UniValue setzambkseed(const UniValue& params, bool fHelp)
// {
//     if(fHelp || params.size() != 1)
//         throw std::runtime_error(
//             "setzambkseed \"seed\"\n"
//             "\nSet the wallet's deterministic zambk seed to a specific value.\n" +
//             HelpRequiringPassphrase() + "\n"

//             "\nArguments:\n"
//             "1. \"seed\"        (string, required) The deterministic zambk seed.\n"

//             "\nResult\n"
//             "\"success\" : b,  (boolean) Whether the seed was successfully set.\n"

//             "\nExamples\n" +
//             HelpExampleCli("setzambkseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5") +
//             HelpExampleRpc("setzambkseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5"));
...
//     CzAMBKWallet* zwallet = pwalletMain->getZWallet();
...
// UniValue getzambkseed(const UniValue& params, bool fHelp)
// {
//     if(fHelp || !params.empty())
//         throw std::runtime_error(
//             "getzambkseed\n"
...
//             "\nResult\n"
//             "\"seed\" : s,  (string) The deterministic zAMBK seed.\n"

//             "\nExamples\n" +
//             HelpExampleCli("getzambkseed", "") + HelpExampleRpc("getzambkseed", ""));
...
UniValue generatemintlist(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 2)
        throw std::runtime_error(
            "generatemintlist\n"
            "\nShow mints that are derived from the deterministic zAMBK seed.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments\n"
            "1. \"count\"  : n,  (numeric) Which sequential zAMBK to start with.\n"
            "2. \"range\"  : n,  (numeric) How many zAMBK to generate.\n"
    ...
    CzAMBKWallet* zwallet = pwalletMain->zwalletMain;
...
// UniValue dzambkstate(const UniValue& params, bool fHelp) {
//     if (fHelp || params.size() != 0)
//         throw std::runtime_error(
//                 "dzambkstate\n"
//                         "\nThe current state of the mintpool of the deterministic zAMBK wallet.\n" +
//                 HelpRequiringPassphrase() + "\n"
...
//     obj.push_back(Pair("dzambk_count", nCount));
...
void static SearchThread(CzAMBKWallet* zwallet, int nCountStart, int nCountEnd)
{
    ...
    try {
        ...
        for(int i = nCountStart; i < nCountEnd; i++) {
            ...
            CKey key;
            zwallet->SeedToZAMBK(zerocoinSeed, bnValue, bnSerial, bnRandomness, key);
...
// UniValue searchdzambk(const UniValue& params, bool fHelp)
// {
//     if(fHelp || params.size() != 3)
//         throw std::runtime_error(
//             "searchdzambk\n"
//             "\nMake an extended search for deterministically generated zAMBK that have not yet been recognized by the wallet.\n" +
//             HelpRequiringPassphrase() + "\n"

//             "\nArguments\n"
//             "1. \"count\"       (numeric) Which sequential zAMBK to start with.\n"
//             "2. \"range\"       (numeric) How many zAMBK to generate.\n"
//             "3. \"threads\"     (numeric) How many threads should this operation consume.\n"

//             "\nExamples\n" +
//             HelpExampleCli("searchdzambk", "1, 100, 2") + HelpExampleRpc("searchdzambk", "1, 100, 2"));
...
//     boost::thread_group* dzambkThreads = new boost::thread_group();
...
//         nPrevThreadEnd = nEnd;
//         dzambkThreads->create_thread(boost::bind(&SearchThread, zwallet, nStart, nEnd));
//     }

//     dzambkThreads->join_all();

//     zwallet->RemoveMintsFromPool(pwalletMain->zambkTracker->GetSerialHashes());
...
UniValue createautomintaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
                ...
                "\nResult\n"
                "\"address\"     (string) AMBANKCOIN address for auto minting\n" +
...
//             "5. \"address\"          (string, optional) AMBANKCOIN address to spend to. If not specified, spend to change add.\n"
...
//         throw std::runtime_error(
//             "clearspendcache\n"
//             "\nClear the pre-computed zAMBK spend cache, and database.\n" +
//             HelpRequiringPassphrase() + "\n"
...
//             if (fLocked) {
//                 if (zambkTracker->ClearSpendCache()) {
