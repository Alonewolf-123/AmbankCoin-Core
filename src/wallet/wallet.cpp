...
#include "zambk/zambkwallet.h"
#include "zambk/zambktracker.h"
#include "zambk/deterministicmint.h"
#include <assert.h>
...
bool CWallet::AddToWallet(const CWalletTx& wtxIn, bool fFromLoadWallet, CWalletDB* pwalletdb)
{
    ...
    if (fFromLoadWallet) {
        ...
        // Break debit/ambk balance caches:
        wtx.MarkDirty();
...
{
    return zambkTracker->HasSerial(bnSerial);
}
...
CAmount CWalletTx::GetUnspentAmbk(const isminefilter& filter) const
{
    ...
    CAmount ambk = 0;
    if (filter & ISMINE_SPENDABLE) {
        ambk += pwallet->GetAmbk(*this, ISMINE_SPENDABLE, true);
    }
    if (filter & ISMINE_WATCH_ONLY) {
        ambk += pwallet->GetAmbk(*this, ISMINE_WATCH_ONLY, true);
    }
    if (filter & ISMINE_COLD) {
        ambk += pwallet->GetAmbk(*this, ISMINE_COLD, true);
    }
    if (filter & ISMINE_SPENDABLE_DELEGATED) {
        ambk += pwallet->GetAmbk(*this, ISMINE_SPENDABLE_DELEGATED, true);
    }
    return ambk;
}

CAmount CWalletTx::GetAmbk(const isminefilter& filter) const
{
    CAmount ambk = 0;
    if (filter & ISMINE_SPENDABLE) {
        ...
        if (fAmbkCached)
            ...
            nAmbkCached = pwallet->GetAmbk(*this, ISMINE_SPENDABLE, false);
            fAmbkCached = true;
            ambk += nAmbkCached;
        }
    }
    if (filter & ISMINE_WATCH_ONLY) {
        if (fWatchAmbkCached)
            ambk += nWatchAmbkCached;
        else {
            nWatchAmbkCached = pwallet->GetAmbk(*this, ISMINE_WATCH_ONLY, false);
            fWatchAmbkCached = true;
            ambk += nWatchAmbkCached;
        }
    }
    if (filter & ISMINE_COLD) {
        if (fColdAmbkCached)
            ambk += nColdAmbkCached;
        else {
            nColdAmbkCached = pwallet->GetAmbk(*this, ISMINE_COLD, false);
            fColdAmbkCached = true;
            ambk += nColdAmbkCached;
        }
    }
    if (filter & ISMINE_SPENDABLE_DELEGATED) {
        if (fDelegatedAmbkCached)
            ambk += nDelegatedAmbkCached;
        else {
            nDelegatedAmbkCached = pwallet->GetAmbk(*this, ISMINE_SPENDABLE_DELEGATED, false);
            fDelegatedAmbkCached = true;
            ambk += nDelegatedAmbkCached;
        }
    }
    return ambk;
...
CAmount CWalletTx::GetImmatureAmbk(bool fUseCache, const isminefilter& filter) const
{
    LOCK(cs_main);
    if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0 && IsInMainChain()) {
        if (fUseCache && fImmatureAmbkCached && filter == ISMINE_SPENDABLE_ALL)
            return nImmatureAmbkCached;
        nImmatureAmbkCached = pwallet->GetAmbk(*this, filter);
        fImmatureAmbkCached = true;
        return nImmatureAmbkCached;
    }
...
CAmount CWalletTx::GetAvailableAmbk(bool fUseCache) const
{
    return GetUnspentAmbk(ISMINE_SPENDABLE_ALL);
}

CAmount CWalletTx::GetColdStakingAmbk(bool fUseCache) const
{
    return GetUnspentAmbk(ISMINE_COLD);
}

CAmount CWalletTx::GetStakeDelegationAmbk(bool fUseCache) const
{
    return GetUnspentAmbk(ISMINE_SPENDABLE_DELEGATED);
}

CAmount CWalletTx::UpdateAmount(CAmount& amountToUpdate, bool& cacheFlagToUpdate, bool fUseCache, isminetype mimeType, bool fAmbk) const
{
    if (pwallet == 0)
        return 0;
    ...
    amountToUpdate = (fAmbk) ? GetAmbk(mimeType) : GetDebit(mimeType);
    cacheFlagToUpdate = true;
    return amountToUpdate;
}

// Return sum of unlocked coins
CAmount CWalletTx::GetUnlockedAmbk() const
{
    if (pwallet == 0)
        return 0;
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        nAmbk += pwallet->GetAmbk(txout, ISMINE_SPENDABLE);
        if (!MoneyRange(nAmbk))
            throw std::runtime_error("CWalletTx::GetUnlockedAmbk() : value out of range");
    }
...
CAmount CWalletTx::GetLockedAmbk() const
{
    if (pwallet == 0)
        return 0;
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        // Add delegated coins
        nAmbk += pwallet->GetAmbk(txout, ISMINE_SPENDABLE_DELEGATED);
        ...
        if (!MoneyRange(nAmbk))
            throw std::runtime_error("CWalletTx::GetLockedAmbk() : value out of range");
    }

    return nAmbk;
}

CAmount CWalletTx::GetImmatureWatchOnlyAmbk(const bool& fUseCache) const
{
    LOCK(cs_main);
    if (IsCoinBase() && GetBlocksToMaturity() > 0 && IsInMainChain()) {
        if (fUseCache && fImmatureWatchAmbkCached)
            return nImmatureWatchAmbkCached;
        nImmatureWatchAmbkCached = pwallet->GetAmbk(*this, ISMINE_WATCH_ONLY);
        fImmatureWatchAmbkCached = true;
        return nImmatureWatchAmbkCached;
    }
...
CAmount CWalletTx::GetAvailableWatchOnlyAmbk(const bool& fUseCache) const
{
    if (pwallet == 0)
        return 0;
    ...
    if (fUseCache && fAvailableWatchAmbkCached)
        return nAvailableWatchAmbkCached;

    CAmount nAmbk = 0;
    for (unsigned int i = 0; i < vout.size(); i++) {
        if (!pwallet->IsSpent(GetHash(), i)) {
            const CTxOut& txout = vout[i];
            nAmbk += pwallet->GetAmbk(txout, ISMINE_WATCH_ONLY);
            if (!MoneyRange(nAmbk))
                throw std::runtime_error("CWalletTx::GetAvailableAmbk() : value out of range");
        }
    }

    nAvailableWatchAmbkCached = nAmbk;
    fAvailableWatchAmbkCached = true;
    return nAmbk;
}

CAmount CWalletTx::GetLockedWatchOnlyAmbk() const
{
    if (pwallet == 0)
        return 0;
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate, bool fromStartup)
{
    ...
    bool fCheckZAMBK = GetBoolArg("-zapwallettxes", false);
    if (fCheckZAMBK)
        zambkTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            //If this is a zapwallettx, need to readd zambk
            if (fCheckZAMBK && pindex->nHeight >= Params().Zerocoin_StartHeight()) {
...
CAmount CWallet::GetColdStakingBalance() const
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
        if (pcoin.HasP2CSOutputs() && pcoin.IsTrusted())
            nTotal += pcoin.GetColdStakingAmbk();
    });
}
...
CAmount CWallet::GetDelegatedBalance() const
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            if (pcoin.HasP2CSOutputs() && pcoin.IsTrusted())
                nTotal += pcoin.GetStakeDelegationAmbk();
    });
}
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zAMBK in the future, for now it's useless,
        ...
        CAmount nBalance = 0;
        std::vector<CMintMeta> vMints = zambkTracker->GetMints(true);
    ...
    return zambkTracker->GetBalance(false, false);
}
...
{
    return zambkTracker->GetUnconfirmedBalance();
}
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        std::set<CMintMeta> setMints = zambkTracker->ListMints(true, true, true);
...
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            nTotal += pcoin.GetImmatureAmbk(false);
    });
}
...
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            nTotal += pcoin.GetImmatureAmbk(false, ISMINE_COLD);
    });
}
...
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            nTotal += pcoin.GetImmatureAmbk(false, ISMINE_SPENDABLE_DELEGATED);
    });
}
...
CAmount CWallet::GetWatchOnlyBalance() const
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            if (pcoin.IsTrusted())
                nTotal += pcoin.GetAvailableWatchOnlyAmbk();
    });
}
...
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            nTotal += pcoin.GetImmatureWatchOnlyAmbk();
    });
}
...
CAmount CWallet::GetLockedWatchOnlyBalance() const
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
            if (pcoin.IsTrusted() && pcoin.GetDepthInMainChain() > 0)
                nTotal += pcoin.GetLockedWatchOnlyAmbk();
    });
}
        ...
        int blockHeight, bool fPrecompute)
{
    ...
    if (GetBoolArg("-ambkstake", true) && !fPrecompute) {
        for (const COutput &out : vCoins) {
            ...
            std::unique_ptr<CAmbkStake> input(new CAmbkStake());
            input->SetInput((CTransaction) *out.tx, out.i);
            listInputs.emplace_back(std::move(input));
        }
    }

    /* Disable zAMBK Staking
    //zAMBK
    if ((GetBoolArg("-zambkstake", true) || fPrecompute) && chainActive.Height() > Params().Zerocoin_Block_V2_Start() && !sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        //Only update zAMBK set once per update interval
        bool fUpdate = false;
        ...
        std::set<CMintMeta> setMints = zambkTracker->ListMints(true, true, fUpdate);
...
bool CWallet::MintableCoins()
{
    ...
    CAmount nZambkBalance = GetZerocoinBalance(false);
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 AMBK collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<std::pair<CScript, CAmount> > vecSend;
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 AMBK collateral
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX, false, fIncludeDelegated)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10,000,000 AMBK.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 10,000,000 AMBK.");
                    } else {
                ...
                for (PAIRTYPE(const CWalletTx*, unsigned int) pcoin : setCoins) {
                    ...
                    CAmount nAmbk = pcoin.first->vout[pcoin.second].nValue;
        ...
        )
{
    ...
    for (std::unique_ptr<CStakeInput>& stakeInput : listInputs) {
        ...
        if (Stake(pindexPrev, stakeInput.get(), nBits, nTxNewTime, hashProofOfStake)) {
            ...
            if (!stakeInput->CreateTxOuts(this, vout, nAmbk)) {
            ...
            if (!stakeInput->IsZAMBK()) {
                ...
                CAmount nRemaining = nAmbk - nMinFee;
                if (outputs > 1) {
            ...
            //Masternode payment
            FillBlockPayee(txNew, nMinFee, true, stakeInput->IsZAMBK());

            {
                TRY_LOCK(zambkTracker->cs_spendcache, fLocked);
                if (!fLocked)
                    continue;
            ...
            //Mark mints as spent
            if (stakeInput->IsZAMBK()) {
                CZAmbkStake* z = (CZAmbkStake*)stakeInput.get();
...
void CWallet::CreateAutoMintTransaction(const CAmount& nMintAmount, CCoinControl* coinControl)
{
    if (nMintAmount > 0){
        ...
        LogPrintf("CWallet::AutoZeromint() @ block %ld: successfully minted %ld zAMBK. Current percentage of zAMBK: %lf%%\n",
...
void CWallet::AutoZeromint()
{
    ...
    // zAMBK are integers > 0, so we can't mint 10% of 9 AMBK
    if (nBalance < 10){
        LogPrint("zero", "CWallet::AutoZeromint(): available balance (%ld) too small for minting zAMBK\n", nBalance);
        return;
    }

    // Percentage of zAMBK we already have
    ...
    if(dPercentage >= nZeromintPercentage){
        LogPrint("zero", "CWallet::AutoZeromint() @block %ld: percentage of existing zAMBK (%lf%%) already >= configured percentage (%d%%). No minting needed...\n",
                  chainActive.Tip()->nHeight, dPercentage, nZeromintPercentage);
        return;
    }

    // zAMBK amount needed for the target percentage
    nToMintAmount = ((nZerocoinBalance + nBalance) * nZeromintPercentage / 100);

    // zAMBK amount missing from target (must be minted)
    nToMintAmount = (nToMintAmount - nZerocoinBalance) / COIN;

    // Use the biggest denomination smaller than the needed zAMBK We'll only mint exact denomination to make minting faster.
    ...
    // If a preferred denomination is used (means nPreferredDenom != 0) do nothing until we have enough AMBK to mint this denomination
...
bool CWallet::MultiSend()
{
    ...
    for (const COutput& out : vCoins) {
        ...
        for (unsigned int i = 0; i < vMultiSend.size(); i++) {
            ...
            nAmount = ((out.tx->GetAmbk(filter) - out.tx->GetDebit(filter)) * vMultiSend[i].second) / 100;
...
bool CWallet::CreateZAMBKOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwalletMain->GenerateDeterministicZAMBK(denomination, coin, dMint);
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, std::vector<CDeterministicMint>& vDMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    while (true) {
        ...
        if (!CreateZAMBKOutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new zambk output", __func__);
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to Ambk
    ...
    // Sign if these are ambankcoin outputs - NOTE that zAMBK outputs are signed later in SoK
    if (!isZCSpendChange) {
        int nIn = 0;
        for (const std::pair<const CWalletTx*, unsigned int>& coin : setCoins) {
            if (!SignSignature(*this, *coin.first, txNew, nIn++)) {
...
bool CWallet::CheckCoinSpend(libzerocoin::CoinSpend& spend, libzerocoin::Accumulator& accumulator, CZerocoinSpendReceipt& receipt)
{
    if (!spend.Verify(accumulator)) {
        receipt.SetStatus(_("The transaction did not verify"), ZAMBK_BAD_SERIALIZATION);
    ...
    if (Params().NetworkID() != CBaseChainParams::REGTEST && IsSerialKnown(spend.getCoinSerialNumber())) {
        //Tried to spend an already spent zAMBK
        receipt.SetStatus(_("The coin spend has been used"), ZAMBK_SPENT_USED_ZAMBK);
        uint256 hashSerial = GetSerialHash(spend.getCoinSerialNumber());
        if(!zambkTracker->HasSerialHash(hashSerial))
            return error("%s: serialhash %s not found in tracker", __func__, hashSerial.GetHex());

        CMintMeta meta = zambkTracker->Get(hashSerial);
        meta.isUsed = true;
        if (!zambkTracker->UpdateState(meta))
                         ...
                         CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    receipt.SetStatus(_("Transaction Mint Started"), ZAMBK_TXMINT_GENERAL);
    ...
    while (nLockAttempts < 100) {
        TRY_LOCK(zambkTracker->cs_spendcache, lockSpendcache);
        if (!lockSpendcache) {
        ...
        for (auto &it : mapMintsSelected) {
            ...
            CMintMeta meta = zambkTracker->Get(GetSerialHash(mint.GetSerialNumber()));
            CoinWitnessData *coinWitness = zambkTracker->GetSpendCache(meta.hashStake);
            ...
            if (!GenerateAccumulatorWitness(coinWitness, mapAccumulators, pindexCheckpoint)) {
                receipt.SetStatus(_("Couldn't generate the accumulator witness"),
                                  ZAMBK_FAILED_ACCUMULATOR_INITIALIZATION);
            ...
            if (nVersion >= libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (!mint.GetKeyPair(key))
                    return error("%s: failed to set zAMBK privkey mint version=%d", __func__, nVersion);
            ...
            try {
                ...
                if (!CheckCoinSpend(spend, accumulator, receipt)) {
                    receipt.SetStatus(_("CoinSpend: failed check"), ZAMBK_SPEND_ERROR);
                    return error("%s : %s", __func__, receipt.GetStatusMessage());
                }
                ...
                receipt.SetStatus(_("CoinSpend: Accumulator witness does not verify"), ZAMBK_INVALID_WITNESS);
                return error("%s : %s", __func__, receipt.GetStatusMessage());
            }
        }
        break;
    ...
    if (nLockAttempts == 100) {
        ...
        receipt.SetStatus(_("could not get lock on cs_spendcache"), ZAMBK_TXMINT_GENERAL);
        return false;
    }
    ...
    receipt.SetStatus(_("Spend Valid"), ZAMBK_SPEND_OKAY); // Everything okay
    ...
    while (nLockAttempts < 100) {
        ...
        for (auto &it : mapMintsSelected) {
            ...
            if (!GetTransaction(mint.GetTxHash(), txMint, hashBlock)) {
                receipt.SetStatus(strprintf(_("Unable to find transaction containing mint %s"), mint.GetTxHash().GetHex()), ZAMBK_TXMINT_GENERAL);
                return false;
            } else if (mapBlockIndex.count(hashBlock) < 1) {
                // check that this mint made it into the blockchain
                receipt.SetStatus(_("Mint did not make it into blockchain"), ZAMBK_TXMINT_GENERAL);
                return false;
            }
            ...
            if (outputIndex == -1) {
                receipt.SetStatus(_("Pubcoin not found in mint tx"), ZAMBK_TXMINT_GENERAL);
                return false;
            }

            mint.SetOutputIndex(outputIndex);
            CTxIn in;
            if(!ZAMBKModule::createInput(in, mint, hashTxOut, spendVersion)) {
                receipt.SetStatus(_("Cannot create public spend input"), ZAMBK_TXMINT_GENERAL);
                return false;
            }
            vin.emplace_back(in);
        ...
        bool isPublicSpend)
{
    ...
    int nStatus = ZAMBK_TRX_FUNDS_PROBLEMS;
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zAMBK"), nStatus);
        return false;
    }

    // Create transaction
    nStatus = ZAMBK_TRX_CREATE;
    ...
    const int nMaxSpends = Params().Zerocoin_MaxPublicSpendsPerTransaction(); // Maximum possible spends for one zAMBK public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the zAMBK used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
        setMints = zambkTracker->ListMints(true, false, true, true); // need to find mints to spend
        if(setMints.empty()) {
        ...
        // Select the zAMBK mints to use in this spend
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            for (std::pair<CBitcoinAddress*,CAmount> pair : addressesTo){
                ...
                //add output to ambankcoin address to the transaction (the actual primary spend taking place)
                // TODO: check value?
            ...
            if (nBytes >= MAX_ZEROCOIN_TX_SIZE) {
                receipt.SetStatus(_("In rare cases, a spend with 7 coins exceeds our maximum allowable transaction size, please retry spend using 6 or less coins"), ZAMBK_TX_TOO_LARGE);
                return false;
            }
    ...
    receipt.SetStatus(_("Transaction Created"), ZAMBK_SPEND_OKAY); // Everything okay
...
std::string CWallet::ResetMintZerocoin()
{
    ...
    std::set<CMintMeta> setMints = zambkTracker->ListMints(false, false, true);
...
void CWallet::ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored, std::list<CDeterministicMint>& listDMintsRestored)
{
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!zambkTracker->UnArchive(hashPubcoin, false)) {
            LogPrintf("%s : failed to unarchive mint %s\n", __func__, mint.GetValue().GetHex());
        } else {
            zambkTracker->UpdateZerocoinMint(mint);
        }
    ...
    for (CDeterministicMint dMint : listDMints) {
        ...
        if (!zambkTracker->UnArchive(dMint.GetPubcoinHash(), true)) {
            LogPrintf("%s : failed to unarchive deterministic mint %s\n", __func__, dMint.GetPubcoinHash().GetHex());
        } else {
            zambkTracker->Add(dMint, true);
        }
        listDMintsRestored.emplace_back(dMint);
    }
}

std::string CWallet::GetUniqueWalletBackupName(bool fzambkAuto) const
{
    ...
    return strprintf("wallet%s.dat%s", fzambkAuto ? "-autozambkbackup" : "", DateTimeStrFormat(".%Y-%m-%d-%H-%M", GetTime()));
}

void CWallet::ZAmbkBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        strNewBackupName = strprintf("wallet-autozambkbackup-%d.dat", i);
        ...
        if (boost::filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                boost::filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozambkbackup-%d.dat", i + 1);
            ...
            strNewBackupName = strprintf("wallet-autozambkbackup-%d.dat", 0);
            backupPath = backupDir / strNewBackupName;
            break;
        }
    ...
    if(!GetArg("-zambkbackuppath", "").empty()) {
        boost::filesystem::path customPath(GetArg("-zambkbackuppath", ""));
...
)
{
    ...
    int nStatus = ZAMBK_SPEND_ERROR;

    if (IsLocked()) {
        receipt.SetStatus("Error: Wallet locked, unable to create transaction!", ZAMBK_WALLET_LOCKED);
        return false;
    }
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        nStatus = ZAMBK_COMMIT_FAILED;

        //reset all mints
        for (CZerocoinMint mint : vMintsSelected) {
            uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
            zambkTracker->SetPubcoinNotUsed(hashPubcoin);
        ...
        for (CZerocoinSpend spend : receipt.GetSpends()) {
            if (!walletdb.EraseZerocoinSpendSerialEntry(spend.GetSerial())) {
                receipt.SetStatus("Error: It cannot delete coin serial number in wallet", ZAMBK_ERASE_SPENDS_FAILED);
            }
        ...
        for (auto& dMint : vNewMints) {
            if (!walletdb.EraseDeterministicMint(dMint.GetPubcoinHash())) {
                receipt.SetStatus("Error: Unable to cannot delete zerocoin mint in wallet", ZAMBK_ERASE_NEW_MINTS_FAILED);
            }
        }
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        zambkTracker->SetPubcoinUsed(hashPubcoin, txidSpend);

        CMintMeta metaCheck = zambkTracker->GetMetaFromPubcoin(hashPubcoin);
        if (!metaCheck.isUsed) {
    ...
    receipt.SetStatus("Spend Successful", ZAMBK_SPEND_OKAY);  // When we reach this point spending zAMBK was successful
...
{
    CMintMeta meta;
    if (!zambkTracker->GetMetaFromStakeHash(hashStake, meta))
...
{
    if (zambkTracker->HasPubcoin(bnValue))
        return true;
...
bool CWallet::UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CZerocoinMint mint;
    if (zambkTracker->HasPubcoinHash(hashValue)) {
        CMintMeta meta = zambkTracker->GetMetaFromPubcoin(hashValue);
        meta.nHeight = nHeight;
        meta.txid = txid;
        return zambkTracker->UpdateState(meta);
    } else {
...
bool CWallet::SetMintUnspent(const CBigNum& bnSerial)
{
    ...
    CMintMeta meta = zambkTracker->Get(hashSerial);
    zambkTracker->SetPubcoinNotUsed(meta.hashPubcoin);
    return true;
}
...
{
    // We don't even need to worry about this code.. no zAMBK.
    /*
    LogPrintf("Precomputer started\n");
    RenameThread("ambankcoin-precomputer");
    ...
    while (true) {
        ...
        for (std::unique_ptr <CStakeInput>& stakeInput : listInputs) {
...

            {
                ...
                CoinWitnessData* witnessData = zambkTracker->GetSpendCache(serialHash);
...
void CWallet::setZWallet(CzAMBKWallet* zwallet)
{
    zwalletMain = zwallet;
    zambkTracker = std::unique_ptr<CzAMBKTracker>(new CzAMBKTracker(strWalletFile));
}

CzAMBKWallet* CWallet::getZWallet()
{
    return zwalletMain;
}
...
void CWallet::setZAmbkAutoBackups(bool fEnabled)
{
...
CAmount CWallet::GetAmbk(const CTxOut& txout, const isminefilter& filter) const
{
    if (!MoneyRange(txout.nValue))
        throw std::runtime_error("CWallet::GetAmbk() : value out of range");
...
CAmount CWallet::GetAmbk(const CTransaction& tx, const isminefilter& filter, const bool fUnspent) const
{
    CAmount nAmbk = 0;
    for (int i = 0; i < tx.vout.size(); i++) {
        if (fUnspent && IsSpent(tx.GetHash(), i)) continue;
        nAmbk += GetAmbk(tx.vout[i], filter);
    }
    if (!MoneyRange(nAmbk))
        throw std::runtime_error("CWallet::GetAmbk() : value out of range");
    return nAmbk;
...
void CWalletTx::Init(const CWallet* pwalletIn)
{
    ...
    fDebitCached = false;
    fAmbkCached = false;
    fImmatureAmbkCached = false;
    fAvailableAmbkCached = false;
    fAnonymizableAmbkCached = false;
    fAnonymizedAmbkCached = false;
    fDenomUnconfAmbkCached = false;
    fDenomConfAmbkCached = false;
    fWatchDebitCached = false;
    fWatchAmbkCached = false;
    fImmatureWatchAmbkCached = false;
    fAvailableWatchAmbkCached = false;
    fChangeCached = false;
    fColdDebitCached = false;
    fColdAmbkCached = false;
    fDelegatedDebitCached = false;
    fDelegatedAmbkCached = false;
    fStakeDelegationVoided = false;
    nDebitCached = 0;
    nAmbkCached = 0;
    nImmatureAmbkCached = 0;
    nAvailableAmbkCached = 0;
    nAnonymizableAmbkCached = 0;
    nAnonymizedAmbkCached = 0;
    nDenomUnconfAmbkCached = 0;
    nDenomConfAmbkCached = 0;
    nWatchDebitCached = 0;
    nWatchAmbkCached = 0;
    nAvailableWatchAmbkCached = 0;
    nImmatureWatchAmbkCached = 0;
    nColdDebitCached = 0;
    nColdAmbkCached = 0;
    nDelegatedDebitCached = 0;
    nDelegatedAmbkCached = 0;
    nChangeCached = 0;
    nOrderPos = -1;
}
