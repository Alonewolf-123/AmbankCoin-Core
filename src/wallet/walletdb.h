...
public:
    ...
    CAmount GetAccountAmbkDebit(const std::string& strAccount);
    void ListAccountAmbkDebit(const std::string& strAccount, std::list<CAccountingEntry>& acentries);
    ...
    bool WriteZAMBKSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZAMBKSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZAMBKSeed_deprecated(uint256& seed);
    bool EraseZAMBKSeed();
    bool EraseZAMBKSeed_deprecated();

    bool WriteZAMBKCount(const uint32_t& nCount);
    bool ReadZAMBKCount(uint32_t& nCount);
