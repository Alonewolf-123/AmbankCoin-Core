...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zambkspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10,000,000 AMBK at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zAMBK send
enum ZerocoinSpendStatus {
    ZAMBK_SPEND_OKAY = 0,                            // No error
    ZAMBK_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZAMBK_WALLET_LOCKED = 2,                         // Wallet was locked
    ZAMBK_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZAMBK_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZAMBK_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZAMBK_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZAMBK_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZAMBK_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZAMBK_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZAMBK_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZAMBK_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZAMBK_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZAMBK_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZAMBK_SPENT_USED_ZAMBK = 14,                      // Coin has already been spend
    ZAMBK_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZAMBK_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZAmbkBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZAMBKOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzambkAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzAMBKTracker> zambkTracker;
    ...
    void setZWallet(CzAMBKWallet* zwallet);
    CzAMBKWallet* getZWallet();
    bool isZeromintEnabled();
    void setZAmbkAutoBackups(bool fEnabled);
    ...
    /// Get 10,000,000 AMBK output and keys which can be used for the Masternode
    ...
    CAmount GetAmbk(const CTxOut& txout, const isminefilter& filter) const;
    ...
    CAmount GetAmbk(const CTransaction& tx, const isminefilter& filter, const bool fUnspent = false) const;
    ...
    /** zAMBK reset */
    boost::signals2::signal<void()> NotifyzAMBKReset;
...
public:
    ...
    mutable bool fAmbkCached;
    mutable bool fImmatureAmbkCached;
    mutable bool fAvailableAmbkCached;
    mutable bool fAnonymizableAmbkCached;
    mutable bool fAnonymizedAmbkCached;
    mutable bool fDenomUnconfAmbkCached;
    mutable bool fDenomConfAmbkCached;
    mutable bool fWatchDebitCached;
    mutable bool fWatchAmbkCached;
    mutable bool fImmatureWatchAmbkCached;
    mutable bool fAvailableWatchAmbkCached;
    mutable bool fChangeCached;
    mutable bool fColdDebitCached;
    mutable bool fColdAmbkCached;
    mutable bool fDelegatedDebitCached;
    mutable bool fDelegatedAmbkCached;
    mutable bool fStakeDelegationVoided;
    mutable CAmount nDebitCached;
    mutable CAmount nAmbkCached;
    mutable CAmount nImmatureAmbkCached;
    mutable CAmount nAvailableAmbkCached;
    mutable CAmount nAnonymizableAmbkCached;
    mutable CAmount nAnonymizedAmbkCached;
    mutable CAmount nDenomUnconfAmbkCached;
    mutable CAmount nDenomConfAmbkCached;
    mutable CAmount nWatchDebitCached;
    mutable CAmount nWatchAmbkCached;
    mutable CAmount nImmatureWatchAmbkCached;
    mutable CAmount nAvailableWatchAmbkCached;
    mutable CAmount nChangeCached;
    mutable CAmount nColdDebitCached;
    mutable CAmount nColdAmbkCached;
    mutable CAmount nDelegatedDebitCached;
    mutable CAmount nDelegatedAmbkCached;
    ...
    CAmount GetAmbk(const isminefilter& filter) const;
    CAmount GetUnspentAmbk(const isminefilter& filter) const;
    CAmount GetImmatureAmbk(bool fUseCache = true, const isminefilter& filter = ISMINE_SPENDABLE_ALL) const;
    CAmount GetAvailableAmbk(bool fUseCache = true) const;
    // Return sum of unlocked coins
    CAmount GetUnlockedAmbk() const;
    // Return sum of unlocked coins
    CAmount GetLockedAmbk() const;
    CAmount GetImmatureWatchOnlyAmbk(const bool& fUseCache = true) const;
    CAmount GetAvailableWatchOnlyAmbk(const bool& fUseCache = true) const;
    CAmount GetLockedWatchOnlyAmbk() const;
    CAmount GetChange() const;

    // Cold staking contracts ambk/debit
    CAmount GetColdStakingAmbk(bool fUseCache = true) const;
    CAmount GetColdStakingDebit(bool fUseCache = true) const;
    CAmount GetStakeDelegationAmbk(bool fUseCache = true) const;
    CAmount GetStakeDelegationDebit(bool fUseCache = true) const;

    // Helper method to update the amount and cacheFlag.
    CAmount UpdateAmount(CAmount& amountToUpdate, bool& cacheFlagToUpdate, bool fUseCache, isminetype mimeType, bool fAmbk = true) const;
