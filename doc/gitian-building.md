...
*Setup instructions for a gitian build of AMBANKCOIN Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the AMBANKCOIN
...
to the AMBANKCOIN GitHub release page.
...
- [Building AMBANKCOIN Core](#building-ambankcoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/ambankcoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/ambankcoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/ambankcoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/ambankcoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/ambankcoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/ambankcoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/ambankcoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@ambankcoin.com) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/ambankcoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/ambankcoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/ambankcoin-osx-*-build.assert
```
